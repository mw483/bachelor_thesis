...
\subsubsection{Lagrangian Data Post-Processing and Statistical Reduction}
...
The intensity of the wind fluctuations and the resulting pollutant spread are quantified using Turbulent Kinetic Energy (TKE) and normalized concentration values. \textbf{Turbulent Kinetic Energy ($k$)} TKE represents the mean kinetic energy per unit mass associated with eddies in the turbulent flow. It is derived from the root-mean-square (RMS) of the velocity fluctuations in the streamwise ($u$), spanwise ($v$), and vertical ($w$) directions:

\begin{align}
    u_{rms}^2 &= \overline{u^2} - \bar{u}^2, 
    \quad v_{rms}^2 = \overline{v^2} - \bar{v}^2, 
    \quad w_{rms}^2 = \overline{w^2} - \bar{w}^2 \\ 
    k &= \frac{1}{2} (u_{rms}^2 + v_{rms}^2 + w_{rms}^2)
\end{align}
