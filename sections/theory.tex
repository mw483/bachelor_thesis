...
To simulate urban air pollutant transport, this study employs the Lattice Boltzmann Method (LBM), a mesoscopic computational fluid dynamics (CFD) technique. This mesoscopic approach represents the fluid as an ensemble of virtual parcels. These parcels, or "lattices", are large compared to individual molecules but small from a macroscopic perspective. Because the detailed motion of single molecules has a negligible impact on macroscopic flow properties, the collective behavior of these mesoscopic parcels that are governed by discrete collision and streaming laws can be modeled to accurately recover the macroscopic fluid dynamics \parencite{Hu2025}. The movement of these particles is governed by the Lattice Boltzmann Equation (LBE), which describes the evolution of the discrete-velocity distribution function $f_i$:
...
As noted by \textcite{Kruger2017LBM}, LBM offers several distinct advantages for simulating complex urban environments. The underlying Boltzmann equation is essentially a hyperbolic advection equation where the source terms depend strictly on local values of the distribution function $f$. Because these operations rely only on information from a node and its immediate neighbors, the algorithm is inherently local. This locality facilitates highly efficient parallelization in multi-core and massively parallel architectures, such as the GPUs utilized in this study, which is essential for high-resolution urban simulations.

Furthermore, unlike traditional Finite Volume Methods (FVM), LBM utilizes a regular lattice grid that eliminates the need for complex, body-fitted grid generation around realistic geometries \parencite{Hu2025}. This significantly simplifies preprocessing tasks, as the grid-based nature of the method allows for the efficient handling of irregular urban morphologies and complex building structures without the intensive re-meshing often required by other numerical schemes. \parencite{Hu2025}.

\subsubsection{Lattice Configuration and Accuracy}
Among the many velocity models available, D3Q19 (3 dimensions 19 velocities) and D3Q27 (3 dimensions 27 velocities) are commonly recommended as the benchmark model to use for LBM simulations \parencite{Hu2025}. The choice of the D3Q27 lattice model is fundamental to the accuracy of the turbulence modeling in this research \parencite{Yokouchi2020Simulation}. While the simpler D3Q19 model is often used to reduce computational costs, it lacks the rotational invariance required to capture complex secondary flows accurately. As demonstrated by \textcite{Suga2015D3Q27}, the D3Q27 lattice provides a superior representation of turbulent structures. Their comparative analysis of turbulent pipe flow showed that the D3Q27 model produces significantly more realistic flow fields than the D3Q19 model, which tends to struggle with resolving fine-scale turbulent fluctuations in three-dimensional space. By utilizing 27 discrete velocities, the model can more effectively resolve the secondary circulations and high-frequency fluctuations necessary for an in-depth understanding of the pollutant dispersion mechanism in urban areas.The internal redistribution of particle momentum is governed by the Bhatnagar-Gross-Krook (BGK) collision operator. This operator assumes that the particle populations relax toward a local equilibrium state, $f_i^{eq}$, at a rate controlled by a single relaxation time $\tau$ \parencite{Kruger2017LBM}:
...
\subsubsection{Accuracy of Simulated Pollutant Concentration}
...
% Might not include below in the initial draft due to incomplete data
% \subsection{Source Area Modeling for Scalars and Fluxes}

% In micrometeorological measurements, particularly within complex urban environments, it is critical to determine the spatial representativeness of data collected at a specific height. While traditional models often assume homogeneous surfaces, real-world applications require an understanding of which specific surface areas influence a sensor. \textcite{Schmid1994} addressed this by defining the Source Area as the surface region contributing to a measured scalar concentration or flux, using the concept of a Source Weight Function (SWF).

% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.8\linewidth]{figures/schmidSourceArea.png}
%     \caption{Conceptual illustration of the source area model, adapted from \textcite{Schmid1994}. The 3D surface represents the source weight function, which shows how much each ground location influences the sensor. The influence is small near the sensor, reaches a peak at a certain upwind distance, and then falls off. The shaded region under the horizontal slice (isopleth $f_P$) represents the fraction $P$ of the total influence. The source area $\Omega_P$ is the two-dimensional region on the $x$-$y$ plane formed by the downward projection of this slice.}
%     \label{schmidSourceArea}
% \end{figure}

% The relationship between a measurement at a specific point and the surface sources is fundamentally described by a convolution integral. Let $\eta(\mathbf{r})$ represent a measured quantity at position $\mathbf{r}$, and $Q(\mathbf{r}')$ represent the source strength at position $\mathbf{r}'$. The measurement is linked to the source via a probability transfer function, or Green's function, $f$:

% \begin{equation}
%     \eta(\mathbf{r}) = \int_{R} Q(\mathbf{r}') f(\mathbf{r} - \mathbf{r}') d\mathbf{r}'
% \end{equation}
...
% \begin{equation}
%     \eta(0,0,z_m) = \int_{-\infty}^{\infty} \int_{-\infty}^{x} Q_{\eta}(x, y, z_0) f(-x, -y, z_m - z_0) dx dy
% \end{equation}

% To represent a specific point source at coordinates $(x_s, y_s, z_0)$ with strength $Q_u$, the source term is defined using Dirac delta distributions ($\delta$):

% \begin{equation}
%     Q(x, y, z_0) = Q_u \delta(x - x_s) \delta(y - y_s) \delta(z - z_0)
% \end{equation}
...
% \begin{equation}
%     \eta(0,0,z_m) = Q_u f(x_s, y_s, z_m - z_0)
% \end{equation}
...
% \begin{equation}
%     P = \frac{1}{\Phi_{tot}} \iint_{f(x,y) \ge f_P} f(x,y) dx dy
% \end{equation}

% where the total influence $\Phi_{tot}$ is the integral of $f(x,y)$ over the entire surface:

% \begin{equation}
%     \Phi_{tot} = \iint f(x,y) dx dy
% \end{equation}

% \subsection{Evolution of Flux Footprint Models}

% While analytical models provide a baseline for understanding source areas, modern environmental monitoring requires a balance between accuracy and computational speed. \textcite{Kljun2015} highlights the current challenge in the field, noting:
...
% As described by \textcite{Kljun2015}, there is a clear trade-off in the study of atmospheric dispersion:

% \begin{itemize}
%     \item \textbf{Complex Models:} These include the Large-Eddy Simulations (LES) and Lagrangian Stochastic Particle Dispersion (LPD) models. While they can handle complex urban geometry like street canyons, they require significant supercomputing power and are difficult to use for long-term data.
%     \item \textbf{Analytical Models:} Simplified models like those by \textcite{Schmid1994} are fast and efficient but are often limited to specific atmospheric conditions and heights.
% \end{itemize}
